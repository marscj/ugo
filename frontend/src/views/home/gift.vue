...
export default {
  ...
  methods: {
    onFetch(search, pagination) {
      this.loading = true
      getProductList({category: 6, search: search, sorter:'sort_by', pageNo: pagination.pageNo, pageSize:pagination.pageSize}).then((res) => {
        const { result } = res
        this.data = result
      }).finally(() => {
        this.loading = false
    ...
    onClick(data) {
      this.$router.push({name: 'GiftDetail', params: { id: data.id }})
    },
  },
}
</script>